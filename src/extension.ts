...
export function activate(context: vscode.ExtensionContext) {
	...
	function getState(document: vscode.TextDocument): State {
		...
		let problemSearch = text.match(/Program:\s*(.+)/);
		if (problemSearch !== null && problemSearch.length > 1) {
			problemID = problemSearch[1];
		}
		const state = stateTracker.getState(text, problemID);
